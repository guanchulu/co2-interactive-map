(function () {
  ...
  function merge(boundaries, overlay) {
    ...
    return {
      ...
      features: (boundaries?.features || []).map((feature) => {
        ...
        const props = feature.properties || {};
        const extra = expand(byId[id]);
        const name = extra.city_name_en || extra.city_name || props.city_name_en || props.city_name || `City ${id || "unknown"}`;
        return {
          ...feature,
          properties: {
            ...props,
  ...
  async function run() {
    css();
    await waitBase();
    const [boundaries, overlay, summary] = await Promise.all([
      fetch(BOUNDARY_URL).then((r) => r.json()),
      fetch(OVERLAY_URL).then((r) => r.json()),
      fetch(SUMMARY_URL).then((r) => r.ok ? r.json() : null).catch(() => null),
    ]);
    state.boundaries = merge(boundaries, overlay);
    state.expandedChainSummary = summary || overlay.screening_assumptions || {};
    patchFunctions();
    refreshSearch();
